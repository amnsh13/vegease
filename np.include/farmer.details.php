<?php
  ...
  if (empty($name) || empty($add) || empty($contact) /* || empty($email) || empty($land) || empty($crops)*/){
      header("Location: ../farmernp.html?details=empty");
      exit();
    }else {
     //check if input characters are opcache_invalid
    // if (!preg_match("/^[a-zA-Z]*$/", $name)) {
    //   header("Location: ../farmernp.html?details=invalid");
    //   exit();
     //}else {
       //check if email is opcache_invalid
       //if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
        /// header("Location: ../farmernp.html?details=email");
        // exit();
       //}else {
       ...
       header("Location: ../farmernp.html?details=success");
       exit();
